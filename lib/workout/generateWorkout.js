...
function pickByPattern(exercises, patternWanted, effort, usedPatterns, durationMinutes) {
...
function pickMainMoves(eligible, effort, patterns, durationMinutes) {
  ...
  for (const p of patterns) {
    const ex = pickByPattern(eligible, p, effort, used, durationMinutes);
    if (ex) {
      moves.push(ex);
...
function buildAMRAP(eligible, params, timing) {
  const rawMoves = pickMainMoves(eligible, params.effort, ["push", "pull", "hinge", "core"], params.durationMinutes);
...
function buildEMOM(eligible, params, timing) {
  const rawMoves = pickMainMoves(eligible, params.effort, ["push", "pull", "hinge"], params.durationMinutes);
    ...
    blocks: [
      {
        ...
        moves: toMoves(
          pickMainMoves(eligible, params.effort, ["push", "pull", "hinge", "squat"], params.durationMinutes),
          params.effort
        ),
      }
    ]
    ...
    blocks: [
      {
        ...
        totalRestMin: roundPlan.totalRestMin,
        moves: toMoves(
          pickMainMoves(eligible, params.effort, ["push", "pull", "squat", "core"], params.durationMinutes),
          params.effort
        ),
      }
    ]
    ...
    blocks: [
      {
        ...
        spareMinutes: spare,
        moves: toMoves(
          pickMainMoves(eligible, params.effort, ["push", "pull", "core"], params.durationMinutes),
          params.effort
        ),
      }
    ]
